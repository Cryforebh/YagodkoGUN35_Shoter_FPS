...
public class AiFindWeaponState : AiState
{
    ...
    public void Update(AiAgent agent) {
        // Find pickup - ѕоиск оружи€, если еще не найдено
        if (!pickup) {
        ...
        bool hasWeapon = agent.weapons.Count() >= 1;

        // Wander - Ѕродит, если оружи€ не найденно (в рандомные точки)
        if (!agent.navMeshAgent.hasPath && !agent.navMeshAgent.pathPending || hasWeapon) {
            WorldBounds worldBounds = GameObject.FindObjectOfType<WorldBounds>();
            agent.navMeshAgent.destination = worldBounds.RandomPosition();
        }

        // RunToWeapon - Ќазначает точку пути к найденному оружию и переводит на состо€ние "ƒвижение к цели(оружию)"
        if (hasWeapon)
        {
